<x-app-layout>
    <x-slot name="header">
        <div>
            <h2 class="text-2xl font-bold text-white">Edit {{ $smartComponent->name }}</h2>
    ...
    <div class="p-4 sm:p-6 lg:p-8">
        <div class="max-w-2xl">
            <div class="card p-6 sm:p-8">
                <form method="POST" action="{{ route('smart-components.update', $smartComponent) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                    @include('smart-components._form', ['smartComponent' => $smartComponent])
